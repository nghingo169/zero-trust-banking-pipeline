...
The end-to-end workflow is `full-source-to-gold`, with Bundle resource key `full_source_to_gold`. The earlier `full-source-to-validated-silver` job remains available for compatibility, but this runbook uses the explicitly named source-to-Gold job:
...
Validation checks the Bundle graph and resource configuration. Deployment uploads the source code and creates or updates the four pipelines and both orchestration jobs. Use `full-source-to-gold` for the complete demo; `full-source-to-validated-silver` is retained only for compatibility. The Bundle does not own or remove the catalog and schemas preloaded above.
