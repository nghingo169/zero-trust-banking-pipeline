...
def test_silver_fraud_alert_and_link(test_spark):
    ...
    # 1. Định nghĩa schema chuẩn khớp với kỳ vọng của PySpark
    schema = StructType([
        StructField("alert_id", StringType(), True),
        StructField("account_txn_id", LongType(), True),  # Đổi sang LongType để so sánh <> -1 không bị lỗi Cast
    ...
    data = [
        ...
        ("ALERT_02", -1, "CARD_CLONING", 88.00, "CLOSED", "2026-07-01 11:00:00", "RUN_01")  # Sẽ bị lọc bỏ ở link table
    ]
    df = test_spark.createDataFrame(data, schema)

    with patch("pyspark.sql.readwriter.DataFrameReader.table", return_value=df):
        # Test bảng chính: Giữ lại toàn bộ 2 bản ghi
        ...
        # Test bảng link: Lọc bỏ account_txn_id == -1 (Chỉ còn 1 bản ghi)
...
def test_silver_monitoring_alert_financial_event_union(test_spark):
    ...
    # 1. Mock DataFrame cho Account Link
    ...
    # 2. Mock DataFrame cho Card Link
    ...
    # 3. Phân biệt chính xác tên bảng trong hàm mock (Khắc phục lỗi match nhầm chuỗi)
