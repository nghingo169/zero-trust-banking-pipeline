...
def test_silver_fraud_alert_and_link(test_spark):
    ...
    data = [
        ...
        ("ALERT_02", "-1", "CARD_CLONING", 88.0, "CLOSED", "2026-07-01 11:00:00", "RUN_01")  # Should be filtered out in link table
    ]
    ...
    with patch(f"{target_module}.spark.read.table", return_value=df):
        ...
        assert len(res_alert) == 2  # Main alert table retains all records

        res_link = fincrime_transformation.silver_financial_event_fraud_alert().collect()
        assert len(res_link) == 1   # Link table filters out account_txn_id == -1
