...
def test_duplicate_national_ids_identifies_duplicates(test_spark):
    ...
    target_module = (
        "pipeline.silver.customer_validation"
        if "pipeline.silver.customer_validation" in sys.modules
        else "customer_validation"
    )

    with patch(f"{target_module}.normalize", side_effect=lambda input_df: input_df):
        dup_df = customer_validation.duplicate_national_ids(
            mock_session, "bronze.core_banking_customer"
        )
...
def test_duplicate_national_ids_returns_none_for_non_identity_tables(test_spark):
    ...
    mock_session = MagicMock()
    res = customer_validation.duplicate_national_ids(
        mock_session, "bronze.customer_kyc"
    )
    assert res is None
...
def test_assess_all_rules_passing(test_spark):
    ...
    with patch(f"{target_module}.get_rules", return_value=mock_rules), patch(
...
def test_assess_captures_rule_failures_and_duplicate_national_ids(test_spark):
    ...
    mock_rules = {"cust_no_not_null": "cust_no IS NOT NULL"}
    ...
    with patch(f"{target_module}.get_rules", return_value=mock_rules), patch(
        ...
        res_df = customer_validation.assess(
            df, "core_banking_customer", duplicate_ids=duplicate_ids_df
        )
        ...
        assert rows["CUST_003"]._failed_rule_names == [
            "core_banking_customer__national_id__duplicate"
        ]
        assert (
            "_duplicate_national_id" not in res_df.columns
        )  # Đã bị drop đúng như logic
...
def test_assess_null_evaluations_handled_as_failures(test_spark):
    ...
    mock_rules = {"status_active": "status = 'ACTIVE'"}
