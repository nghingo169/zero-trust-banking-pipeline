...
# ------------------------------------------------------------------------------
import nab_tdm_masking  # Fallback cho đường dẫn đĩa src/
...
def test_masking_referential_integrity_determinism(test_spark):
    ...
    df = test_spark.createDataFrame(
        [("John Smith",), ("John Smith",)],  # Duplicate row to verify determinism
        schema,
    )
...
def test_mask_card_number_rule_1_15(test_spark):
    ...
    assert (
        c16_masked[:9] == c16_orig[:9]
    ), "First 9 digits (BIN 6 + NAB 3) must be retained"
...
def test_mask_national_id_rule_1_12(test_spark):
    ...
    data = [("123456789",), ("123",)]  # Valid >= 9 digits  # Short length fallback
    df = test_spark.createDataFrame(data, schema)

    res_df = df.select(
        F.col("nat_id"), nab_tdm_masking.mask_national_id("nat_id").alias("masked_id")
    )
    rows = res_df.collect()
...
def test_mask_phone_rule_1_13(test_spark):
    ...
    data = [("0412345678",), ("1234",)]  # 10 digits  # Short length
    df = test_spark.createDataFrame(data, schema)

    res_df = df.select(
        F.col("phone"), nab_tdm_masking.mask_phone("phone").alias("masked_phone")
    )
    rows = res_df.collect()
...
def test_mask_name_rule_1_10(test_spark):
    ...
    res_df = df.select(
        F.col("full_name"), nab_tdm_masking.mask_name("full_name").alias("masked_name")
    )
    rows = res_df.collect()
...
def test_mask_address_rule_1_11(test_spark):
    ...
    res_df = df.select(nab_tdm_masking.mask_address("address").alias("masked_address"))
    row = res_df.first()
