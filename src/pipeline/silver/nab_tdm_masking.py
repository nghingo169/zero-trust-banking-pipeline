...
def mask_card_number(original_card: F.Column, mode: str = "spark") -> F.Column:
    ...
    random_digits = F.lpad(
        (F.conv(F.substring(hash_val, 1, 12), 16, 10).cast("long") % 1000000).cast(
            "string"
        ),
        6,
        "0",
    ...
    card_without_check = F.when(
        card_len == 16, F.concat(prefix_9, random_digits)
    ).otherwise(F.concat(prefix_9, F.substring(random_digits, 1, 5)))

    # Append deterministic check digit (not Luhn-compliant per NAB requirement)
    check_digit = F.lpad(
        (F.conv(F.substring(hash_val, 13, 4), 16, 10).cast("long") % 10).cast("string"),
        1,
        "0",
    )
...
def mask_national_id(original_id: F.Column) -> F.Column:
    ...
    middle_digits = F.lpad(
        (F.conv(F.substring(hash_val, 1, 8), 16, 10).cast("long") % 1000).cast(
            "string"
        ),
        3,
        "0",
    ...
    return F.when(id_len >= 9, F.concat(prefix_3, middle_digits, suffix_3)).otherwise(
...
def mask_phone(original_phone: F.Column) -> F.Column:
    ...
    middle_2_digits = F.lpad(
        (F.conv(F.substring(hash_val, 1, 8), 16, 10).cast("long") % 100).cast("string"),
        2,
        "0",
    )

    return F.when(
        phone_len >= 10, F.concat(prefix_4, middle_2_digits, suffix_4)
    ).otherwise(F.concat(F.lit("XXXX"), middle_2_digits, F.lit("XXXX")))
...
def mask_address(original_address: F.Column) -> F.Column:
    ...
    street_num = F.lpad(
        (F.conv(F.substring(hash_val, 1, 8), 16, 10).cast("long") % 999 + 1).cast(
            "string"
        ),
        3,
        "0",
    ...
    postcode_suffix = F.lpad(
        (F.conv(F.substring(hash_val, 9, 8), 16, 10).cast("long") % 1000).cast(
            "string"
        ),
        3,
        "0",
    ...
    return F.concat(
        street_num, F.lit(" Masked Street, MASKED_SUBURB NSW 2"), postcode_suffix
    )
