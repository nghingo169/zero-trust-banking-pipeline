...
def silver_financial_event_risk_score():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("score_id"), "score_id").alias("financial_event_risk_score_key"),
        ...
        F.concat_ws(":", F.lit("account_transaction_risk_score"), F.col("score_id")).alias("bronze_record_ref"),
...
def silver_fraud_alert():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("fraud_alert"), "alert_id").alias("fraud_alert_key"),
        F.col("alert_type"),
        ...
        F.concat_ws(":", F.lit("fraud_alert"), F.col("alert_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_financial_event_fraud_alert():
    df = spark.read.table(validated_src("fraud_alert")).filter("account_txn_id IS NOT NULL AND account_txn_id <> -1")
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("financial_event_fraud_alert"), "alert_id", "account_txn_id").alias("financial_event_fraud_alert_key"),
        hash_key(F.lit("core_banking"), F.lit("ACCOUNT_POSTING"), "account_txn_id").alias("financial_event_key"),
        hash_key(F.lit("fincrime"), F.lit("fraud_alert"), "alert_id").alias("fraud_alert_key"),
        F.lit("fincrime").alias("source_system"),
        F.concat_ws(":", F.col("alert_id"), F.col("account_txn_id")).cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("fraud_alert"), F.col("alert_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_transaction_monitoring_alert():
    ...
    return df.select(
        ...
        F.lit("fincrime").alias("source_system"),
        F.col("alert_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("transaction_monitoring_alert"), F.col("alert_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_monitoring_alert_financial_event():
    df_acc_src = spark.read.table(validated_src("transaction_monitoring_alert_account_transaction")).filter("account_txn_id <> -1")
    ...
    df_card_src = spark.read.table(validated_src("transaction_monitoring_alert_card_transaction")).filter("card_txn_id <> -1")
    ...
    return df_union.select(
        hash_key(F.lit("fincrime"), F.lit("monitoring_alert_financial_event"), "link_id").alias("monitoring_alert_financial_event_key"),
        hash_key(F.lit("fincrime"), F.lit("transaction_monitoring_alert"), "alert_id").alias("monitoring_alert_key"),
        hash_key("src_sys", "evt_type", "txn_id").alias("financial_event_key"),
        F.col("is_primary"),
        F.lit("fincrime").alias("source_system"),
        F.col("link_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("monitoring_alert_link"), F.col("link_id")).alias("bronze_record_ref"),
...
def silver_investigation_case():
    ...
    return df.select(
        ...
        F.concat_ws(":", F.lit("investigation_case"), F.col("case_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_investigation_case_financial_event():
    df_acc_src = spark.read.table(validated_src("investigation_case_account_transaction")).filter("account_txn_id <> -1")
    df_acc = df_acc_src.select(
    ...
    df_card_src = spark.read.table(validated_src("investigation_case_card_transaction")).filter("card_txn_id <> -1")
    df_card = df_card_src.select(
    ...
    return df_union.select(
        hash_key(F.lit("fincrime"), F.lit("case_financial_event"), "link_id").alias("investigation_case_financial_event_key"),
        hash_key(F.lit("fincrime"), F.lit("investigation_case"), "case_id").alias("investigation_case_key"),
        hash_key("src_sys", "evt_type", "txn_id").alias("financial_event_key"),
        F.col("link_reason"),
        F.col("linked_at"),
        F.lit("fincrime").alias("source_system"),
        F.col("link_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("case_txn_link"), F.col("link_id")).alias("bronze_record_ref"),
        F.col("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_investigation_case_fraud_alert():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("case_fraud_alert"), "case_alert_link_id").alias("investigation_case_fraud_alert_key"),
        hash_key(F.lit("fincrime"), F.lit("investigation_case"), "case_id").alias("investigation_case_key"),
        hash_key(F.lit("fincrime"), F.lit("fraud_alert"), "alert_id").alias("fraud_alert_key"),
        F.col("linked_timestamp").cast("timestamp").alias("linked_at"),
        F.lit("fincrime").alias("source_system"),
        F.col("case_alert_link_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("investigation_case_fraud_alert"), F.col("case_alert_link_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_investigation_case_monitoring_alert():
    df = spark.read.table(validated_src("investigation_case_transaction_monitoring_alert"))
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("case_monitoring_alert"), "case_alert_link_id").alias("investigation_case_monitoring_alert_key"),
        hash_key(F.lit("fincrime"), F.lit("investigation_case"), "case_id").alias("investigation_case_key"),
        hash_key(F.lit("fincrime"), F.lit("transaction_monitoring_alert"), "alert_id").alias("monitoring_alert_key"),
        F.col("linked_timestamp").cast("timestamp").alias("linked_at"),
        F.lit("fincrime").alias("source_system"),
        F.col("case_alert_link_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("investigation_case_transaction_monitoring_alert"), F.col("case_alert_link_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_investigation_note():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("investigation_note"), "note_id").alias("investigation_note_key"),
        hash_key(F.lit("fincrime"), F.lit("investigation_case"), "case_id").alias("investigation_case_key"),
        F.col("author_id"),
        ...
        F.concat_ws(":", F.lit("investigation_note"), F.col("note_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_aml_case():
    ...
    return df.select(
        ...
        F.lit("fincrime").alias("source_system"),
        F.col("case_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("aml_case"), F.col("case_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_watchlist_entry():
    ...
    return df.select(
        ...
        F.concat_ws(":", F.lit("watchlist"), F.col("watchlist_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_sanctions_screening():
    ...
    return df.select(
        ...
        F.concat_ws(":", F.lit("sanction_screening"), F.col("screening_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_investigation_case_sanctions_screening():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("case_sanction_screening"), "case_screening_link_id").alias("investigation_case_sanctions_screening_key"),
        hash_key(F.lit("fincrime"), F.lit("investigation_case"), "case_id").alias("investigation_case_key"),
        hash_key(F.lit("fincrime"), F.lit("sanction_screening"), "screening_id").alias("sanctions_screening_key"),
        F.col("linked_timestamp").cast("timestamp").alias("linked_at"),
        F.lit("fincrime").alias("source_system"),
        F.col("case_screening_link_id").cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("investigation_case_sanction_screening"), F.col("case_screening_link_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_suspicious_activity_report():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("suspicious_activity_report"), "sar_id").alias("suspicious_activity_report_key"),
        hash_key(F.lit("fincrime"), F.lit("aml_case"), "case_id").alias("aml_case_key"),
        ...
        F.concat_ws(":", F.lit("suspicious_activity_report"), F.col("sar_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_card_fraud_flag():
    ...
    return df.select(
        hash_key(F.lit("card_system"), F.lit("card_fraud_flag"), "flag_id").alias("card_fraud_flag_key"),
        F.col("flag_reason"),
        ...
        F.concat_ws(":", F.lit("card_fraud_flag"), F.col("flag_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_financial_event_card_fraud_flag():
    df = spark.read.table(validated_src("card_fraud_flag")).filter("card_txn_id IS NOT NULL AND card_txn_id <> -1")
    return df.select(
        hash_key(F.lit("card_system"), F.lit("evt_card_fraud_flag"), "flag_id", "card_txn_id").alias("financial_event_card_fraud_flag_key"),
        hash_key(F.lit("card_system"), F.lit("CARD_PAYMENT"), "card_txn_id").alias("financial_event_key"),
        hash_key(F.lit("card_system"), F.lit("card_fraud_flag"), "flag_id").alias("card_fraud_flag_key"),
        F.lit("card_system").alias("source_system"),
        F.concat_ws(":", F.col("flag_id"), F.col("card_txn_id")).cast("string").alias("source_business_key"),
        F.concat_ws(":", F.lit("card_fraud_flag"), F.col("flag_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_chargeback():
    ...
    return df.select(
        hash_key(F.lit("fincrime"), F.lit("chargeback"), "chargeback_id").alias("chargeback_key"),
        hash_key(F.lit("card_system"), F.lit("CARD_PAYMENT"), "card_txn_id").alias("financial_event_key"),
        F.col("reason_code"),
        ...
        F.concat_ws(":", F.lit("chargeback"), F.col("chargeback_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
...
def silver_call_center_contact():
    ...
    return df.select(
        ...
        F.sha2(F.coalesce(F.col("caller_phone"), F.lit("")), 256).alias("caller_phone_token"),
        ...
        F.concat_ws(":", F.lit("call_center_log"), F.col("call_id")).alias("bronze_record_ref"),
        get_pipeline_run_id(df).alias("pipeline_run_id"),
        F.current_timestamp().alias("ingested_at")
    )
