...
def add_enterprise_cdc_metadata(
    df: DataFrame, domain: str, current_date_str: Optional[str] = None
) -> DataFrame:
    ...
    source_path_col = (
        ...
        or "_metadata" in [c.name for c in df.schema.fields]
        else F.lit("")
    )

    parsed_date = F.regexp_extract(
        source_path_col, r"business_date=(\d{4}-\d{2}-\d{2})", 1
    )
    if current_date_str:
        final_date_col = F.when(parsed_date != "", parsed_date).otherwise(
            F.lit(current_date_str)
        )
    else:
    ...
    return (
        ...
        .withColumn(
            "simulation_id",
            F.when(sim_id_col != "", sim_id_col).otherwise(F.lit("unknown")),
        )
...
# ==============================================================================
def build_cdc_flow(
    table_name: str, domain: str, keys: list[str], cdc_type: str, schema_hints: str
):
    ...
    if cdc_type == "stream":
        ...
        def stream_view():
            ...
            schema_location = (
                f"/Volumes/workspace/bronze/checkpoints/{table_name}_schema"
            )
        ...
        def next_snapshot_and_version(
            ...
            if latest_version is None:
                ...
                remaining_versions = sorted(
                    [v for v in AVAILABLE_BUSINESS_DATES if v > latest_version]
                )
            ...
            exact_snapshot_path = f"{S3_BASE_URL}/simulation_id=*/snapshot_type=full/business_date={date_formatted}/{domain}/{table_name}"
            ...
            try:
                ...
                df = add_enterprise_cdc_metadata(
                    df, domain, current_date_str=date_formatted
                )
                return (df, next_ver)
